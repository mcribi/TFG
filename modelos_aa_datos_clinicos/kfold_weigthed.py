...
for fold, (train_idx, val_idx) in enumerate(kf.split(dataset)):
    ...
    # 3. Oversampling de la clase minoritaria SOLO en train
    ...
    # 5. WeightedRandomSampler para el fold actual (SOLO train)
    def get_sampler(subset):
        subset_labels = labels[subset.indices]
        class_counts = np.bincount(subset_labels)
        weights = 1 / (torch.tensor(class_counts ** 1.7, dtype=torch.float32) + 1e-6)  # Power=1.7
    ...
    # 7. Reinicializar modelo para cada fold
    model = DenseNet121(
        spatial_dims=3,
        in_channels=1,
        out_channels=2,
    ...
    # 8. Entrenamiento del fold
    best_f1 = 0
    for epoch in range(15):  # Aumenta épocas para CV
        model.train()
        epoch_loss = 0
        all_preds, all_labels = [], []
