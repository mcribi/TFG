...
def build_model(
    ...
    # 1️⃣ Crear modelo con 3 canales (por defecto)
    model = r3d_18(weights=None)
    model.fc = nn.Linear(model.fc.in_features, num_classes)

    # 2️⃣ Si queremos 1 canal, sustituir la primera conv3d
    if in_channels == 1:
        print("⚙️ Ajustando la primera capa (stem) para in_channels=1")
    ...
    # 3️⃣ Si hay pesos preentrenados, cargarlos
    if pretrained_weights_path:
        ...
        # ⚠️ Adaptar la primera capa si checkpoint tiene diferente in_channels
        if "stem.0.weight" in state_dict:
            pretrained_stem_weight = state_dict["stem.0.weight"]
            if pretrained_stem_weight.shape[1] != in_channels:
                print(f"⚠️ Ajustando stem.0.weight de {pretrained_stem_weight.shape} a in_channels={in_channels}")
                ...
                # Si checkpoint es (64, 3, 3, 7, 7) y queremos 1 canal
                    ...
                    print("❗️ No se puede adaptar stem automáticamente. Quitando del checkpoint.")
                    del state_dict["stem.0.weight"]

        # ⚠️ Ignorar la capa final fc
        state_dict = {k: v for k, v in state_dict.items() if not k.startswith("fc.")}
        missing, unexpected = model.load_state_dict(state_dict, strict=False)
        print(f"✅ Pesos cargados (missing: {missing}, unexpected: {unexpected})")

    # 4️⃣ Configurar fine-tuning
...
# -------------------- RUN GRID SEARCH --------------------
# Define tus preprocesados (subcarpetas de datos)
preprocessing_dirs = [
...
for prep in preprocessing_dirs:
    ...
    # Crear nuevo dataset para este preprocesado
    ...
    # Lanzar grid de BS, LR y fine-tune
    for bs in batch_sizes_to_try:
        for lr in learning_rates_to_try:
            for ft_mode in fine_tune_modes_to_try:
                ...
                # Añadir columnas con configuración
