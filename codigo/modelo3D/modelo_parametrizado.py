...
def train_model(
    ...
    for epoch in range(epochs):
        ...
        if save_path:
            if epoch == 0:
                # Primera época: guardar SIEMPRE (para que no haya error de que nunca mejora y despues no exista)
                best_val_gmean = val_gmean
                torch.save(model.state_dict(), save_path)
                print(f" Guardado modelo inicial en epoch 1 (G-Mean: {val_gmean:.4f}) en '{save_path}'")
            elif val_gmean > best_val_gmean:
                best_val_gmean = val_gmean
                torch.save(model.state_dict(), save_path)
...
def cross_validate(
    ...
    device='cuda',
    epochs=10,
    save_path_prefix="modelo_densenet_128_fold"
):
    results = []
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(train_val_dataset.indices, labels)):
        ...
        print(f"\n RESULTADOS Fold {fold+1}/{k}")
        print(f"   Accuracy: {acc:.4f} | F1: {f1:.4f} | TPR: {tpr:.4f} | TNR: {tnr:.4f} | G-Mean: {gmean:.4f}")
...
# -------------------- RUN GRID SEARCH --------------------
# Listado de PREPROCESADOS (subcarpetas)
preprocessing_dirs = [
...
for prep in preprocessing_dirs:
    ...
    data_dir = f"../../smaller128/{prep}/npy/images"
    mask_dir = f"../../smaller128/{prep}/npy/masks"

    # Nuevo dataset para este preprocesado
    dataset = LungCTDataset(data_dir, mask_dir, labels_dict_numeric, transform=transform)
    ...
    for bs in batch_sizes_to_try:
        for lr in learning_rates_to_try:
            ...
            df_result = cross_validate(
                ...
                device=device,
                epochs=EPOCHS,
                save_path_prefix=f"modelo_densenet_128_{prep}_bs{bs}_lr{lr}"
            )

            # Añadir columna del preprocesado
...
final_results.to_csv("resultados_grid_search_completo_128.csv", index=False)
print("\n Todos los resultados guardados en 'resultados_grid_search_completo_128.csv'")