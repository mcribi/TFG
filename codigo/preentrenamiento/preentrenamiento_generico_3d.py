...
def preprocess_and_save_3d_series_one_by_one(root_dir, save_dir, target_shape=(256, 512, 512)):
    ...
    print(" Agrupando rutas por series")
    ...
    print(f"Se detectaron {len(series_dicoms)} series CT unicas")
    ...
    for (label, patient_id, series_uid), paths in tqdm(series_dicoms.items()):
        try:
            ...
            del volume, volume_resized, slices  # para liberar memoria

        except Exception as e:
            print(f"Error en {patient_id}-{series_uid}: {e}")
            continue
