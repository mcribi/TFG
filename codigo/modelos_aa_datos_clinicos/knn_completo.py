...
#  escalamos datos necesarios
scaler = StandardScaler()
X_scaled = pd.DataFrame(scaler.fit_transform(X), columns=X.columns)

#  combinaciones de hiperparam
param_grid = [
...
for i, custom_params in enumerate(param_grid, 1):
    ...
    for fold, (train_idx, test_idx) in enumerate(skf.split(X_scaled, y), 1):
        ...
        model = KNeighborsClassifier(
            n_neighbors=custom_params["n_neighbors"],
            weights=custom_params["weights"],
            metric=custom_params["metric"],
            n_jobs=-1
        )
    ...
    # guardamos resultados para esta configuración
...
#  gaurdamos resultados
df_resultados.to_csv("resultados_knn_gridsearch_cv5.csv", index=False)
print("\n Todos los resultados guardados en 'resultados_knn_gridsearch_cv5.csv'")
